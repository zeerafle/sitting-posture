import argparse
import os
import sys
import numpy as np
from sklearn.ensemble import AdaBoostClassifier
from skopt.space import Categorical, Real

current_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.abspath(os.path.join(current_dir, "..", ".."))
sys.path.insert(0, parent_dir)

from models.base_trainer import BaseTrainer
from models.standard_workflow import run_standard_workflow


class AdaBoostStandardTrainer(BaseTrainer):
    def __init__(self):
        super().__init__(
            model_name="adaboost",
            train_combined=True,  # Always use combined data
            use_loso=False        # This is for standard split
        )

    def get_estimator(self):
        return AdaBoostClassifier(
            random_state=self.params["random_state"],
            algorithm='SAMME'
        )

    def get_param_space(self):
        return {
            "n_estimators": Categorical(list(range(
                self.params["adaboost"]["n_estimators_min"],
                self.params["adaboost"]["n_estimators_max"] + 1,
                50
            ))),
            "learning_rate": Real(0.01, 2.0, prior='log-uniform')
        }

    def log_model_specific_metrics(self, model, live):
        """Log AdaBoost-specific metrics."""
        if hasattr(model, 'feature_importances_'):
            live.log_metric("feature_importance_mean", float(np.mean(model.feature_importances_)))
            live.log_metric("feature_importance_std", float(np.std(model.feature_importances_)))
        if hasattr(model, 'estimator_weights_'):
            live.log_metric("estimator_weights_mean", float(np.mean(model.estimator_weights_)))
            live.log_metric("estimator_weights_std", float(np.std(model.estimator_weights_)))

    def run(self):
        """Run the standard workflow."""
        return run_standard_workflow(self)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train AdaBoost model with standard train-test split")
    args = parser.parse_args()

    trainer = AdaBoostStandardTrainer()
    model, metrics = trainer.run()
